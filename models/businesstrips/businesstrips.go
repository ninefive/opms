...
func AddBusinesstrip(upd Businesstrips) error {
	...
	businesstrip.Result = upd.Result
...
func ListBusinesstripApproval(condArr map[string]string, page int, offset int) (num int64, err error, ops []Businesstrips) {
	...
	qb.Select("l.businesstripid", "l.userid", "l.days", "l.changed", "l.approverids", "l.status").From("pms_businesstrips_approver AS la").
	...
	qb.OrderBy("la.approverid").Desc().Limit(offset).Offset(start)
