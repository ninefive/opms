...
func ListOvertime(condArr map[string]string, page, offset int) (num int64, err error, ops []Overtimes) {
	o := orm.NewOrm()
	o.Using("default")
...
//待审批
func ListOvertimeApproval(condArr map[string]string, page, offset int) (num int64, err error, ops []Overtimes) {
	if page < 1 {
		page = 1
	}
	if offset < 1 {
		offset, _ = beego.AppConfig.Int("pagoffset")
	}
	start := (page - 1) * offset
	var overtime []Overtimes
	...
	qb.Select("l.overtimeid", "l.userid", "l.started", "l.ended", "l.longtime", "l.holiday", "l.way", "l.approverids", "la.status").From("pms_overtimes_approver AS la").
		...
		Where("la.userid=?").And("l.status=2")
	...
	qb.OrderBy("la.approverid").
		Desc().Limit(offset).Offset(start)
...
func CountOvertimeApproval(condArr map[string]string) int64 {
	...
	qb.Select("COUNT(1) AS num").From("pms_overtimes_approver AS la").
		LeftJoin("pms_overtimes").On("l.overtimeid=la.overtimeid").
		Where("la.userid=?").
		And("l.status=2")
	if condArr["status"] == "0" {
		qb.And("la.status=0")
