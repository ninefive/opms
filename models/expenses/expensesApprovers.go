...
type ExpensesApprover struct {
	Id        int64 `orm:"pk;colum(approverid);"`
	Expenseid int64
	Userid    int64
	Summary   string
	Status    int
...
func ListExpenseApproverProcessHtml(expenseid int64) string {
	...
	for i, v := range users {
		if v.Avatar == "" {
			avatart = "/status/img/avatar/1.jpg"
		} else {
			avatar = v.Avatar
		}
		if v.Status == 1 {
...
func CheckExpenseApprover(id, userId int64) (int64, int) {
	var expense ExpensesApprover
	o := orm.NewOrm()
	o.QueryTable(models.TablName("expenses_approver")).Filter("expenseid", id).Filter("userid", userId).One(&expense, "approverid", "status")
