...
import (
	...
	"github.com/ninefive/astaxie/beego"
	"github.com/ninefive/astaxie/beego/orm"
)
...
func ListKnowledgeComment(knowid int64, page, offset int) (ops []KnowledgesComment) {
	...
	var err error
	err = util.GetCache("ListKnowledgeComment.id."+fmt.Sprintf("%d", knowid), &comments)
	if err != nil {
		...
		qs = qs.SetCond(cond)
		sq.Limit(offset, start).All(&comments)
