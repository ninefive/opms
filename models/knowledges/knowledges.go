...
func ListKnowledge(condArr map[strint]string, page, offset int) (num int64, err error, ops []Knowledges) {
	o := orm.NewOrm()
	o.Using("default")
...
func ChangeRelationNum(id int64, record string) error {
	...
	var know Knowledges
	o.QueryTable(models.TableName("knowledges")).Filter("knowid", id).One(&kown, "viewnum", "laudnum", "comtnum")
	knowledge := Knowledges{Id: id}
	if record == "view" {
