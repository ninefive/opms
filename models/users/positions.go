...
func GetPositionsName(id int64) string {
	...
	if err != nil {
		...
		name = pos.Name
		utils.SetCache("GetPostionsName.id."+fmt.Sprintf("%d", id), name, cache_expire)
	}
	return name
}
...
func ListPositions(condArr map[string]string, page, offset int) (num int64, err error, ops []Positions) {
	o := orm.NewOrm()
	o.Using("default")
