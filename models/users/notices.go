...
func AddNotices(updNot Notices) error {
	o := orm.NewOrm()
	o.Using("default")
	not := new(Notices)

	not.Id = updNot.Id
	not.Title = updNot.Title
	not.Content = updNot.Content
	not.Status = 1
...
func ListNotices(condArr map[string]string, page, offset int) (num int64, err error, ops []Notices) {
	o := orm.NewOrm()
	o.Using("default")
