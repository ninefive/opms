...
func ListTaskForForm(projectId int64, page, offset int) (ops []ProjectsTask) {
	...
	if err != nil {
		...
		utils.SetCache("ListTaskForForm.id."+fmt.Sprinf("%d", projectId), task, cache_expire)
	}
	return task
}

func ListProjectTask(condArr map[string]string, page, offset int) (num int64, err error, ops []ProjectsTask) {
	o := orm.NewOrm()
	o.Using("default")
	if page < 1 {
		page = 1
	...
	if condArr["projectid"] != "" {
		cond = cond.And("projectid", condArr["prjectid"])
	}
	if condArr["acceptid"] != "" {
