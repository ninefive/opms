...
func UpdateNeeds(id int64, upd ProjectsNeeds) error {
	...
	if upd.Attachment != "" {
		need.Attachment = upd.Attachment
		_, err := o.Update(&needs, "name", "desc", "acceptid", "source", "acceptance", "level", "tasktime", "changed", "stage", "attachment")
		return err
	} else {
...
//列表
func ListProjectNeeds(condArr map[string]string, page, offset int) (num int64, err error, ops []ProjectsNeeds) {
	o := orm.NewOrm()
	o.Using("default")
	if page < 1 {
		page = 1
