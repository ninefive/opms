...
import (
	...
	"github.com/ninefive/opms/models/users"
...
func ListProjectTest(condArr map[string]string, page, offset int) (num int64, err error, ops []ProjectsTest) {
	o := orm.NewOrm()
	o.Using("default")
	if page < 1 {
		page = 1
...
func ChangeProjectTestStatus(id, userid int64, status int, note string) error {
	o := orm.NewOrm()
...
func ChangeProjectTestAccept(id, acceptid, userid int64, note string) error {
	o := orm.NewOrm()
