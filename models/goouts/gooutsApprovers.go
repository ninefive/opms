...
func (this *CooutsApprover) TableName() string {
...
func UpdateGooutsApprover(id int64, upd GooutsApprover) error {
	...
	if err == nil {
		...
		if upd.Status == 2 {
			...
			o.Raw("UPDATE pms_goouts_approver SET status=?,summary=?,changed=? WHERE gooutid=? AND approverid!=?", 2, "前审批人拒绝,后面的审批人默认为拒绝状态", time.Now().Unix(), upd.Gooutid, id).Exec()
		} else {
