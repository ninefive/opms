...
func ListOagood(condArr map[string]string, page, offset int) (num int64, err error, ops []Oagoods) {
	o := orm.NewOrm()
	o.Using("default")
...
func CountOagood(condArr map[string]string) int64 {
	...
	num, _ := qs.SetCond(cond)
	return num
}

//待审批
func ListOagoodApproval(condArr map[string]string, page, offset int) (num int64, err, error, ops []Oagoods) {
	if page < 1 {
		page = 1
	}
	if offset < 1 {
